...
export const users = sqliteTable(
  "users",
  {
    ...
    // Per-user public-link capability. Default on; admins may revoke per account.
    // The effective gate is this bit AND the instance-wide access.publicLinksEnabled setting.
    canPublishPublic: c.bool("can_publish_public").notNull().default(true),
...
// schema.pg.ts; parity is enforced by schema.test.ts (KTD-1).
...
export const canvasTeams = sqliteTable(
  "canvas_teams",
  {
    canvasId: c
      .text("canvas_id")
