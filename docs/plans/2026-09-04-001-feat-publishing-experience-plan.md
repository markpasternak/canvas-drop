...
- Build on the existing stack, brand tokens, and instance-selected skins. Editorial remains the default; no instance setting is changed. Replace the mismatched screenshot-led introduction with sample canvases rendered in the active skin. Keep documentation/tour assets available for existing consumers.
- A marketing sample is a clearly labelled local demonstration with prepared content; no uploads, sign-in bypass, or server mutations. It works by keyboard/tap, remains understandable without JS, and respects reduced motion.
...
Files: landing-page.ts and tests; a small landing demo script and tests; docs/site/assets; brand guidance where current text is stale.

Approach: short hero with a live-looking but explicitly simulated sample canvas, three selectable use cases, a file-to-link demonstration, succinct publish/share/version story, compact capability detail, and team/self-host guidance. Keep SEO, self-hosted fonts, all six primitives plus authoring, legal links, skin propagation, safe escaping, routing, CSP, and signed-in/auth-mode CTA behavior. Use token colors, type and consistent light/dark surfaces; remove carousel dependency from the introductory experience.

Verification: rendered-content/routing/security/skin tests, script tests covering selection/replay and accessible state; browser keyboard/tap, desktop/mobile, light/dark, reduced-motion, and multiple skins. HTML and JS work without external services.
