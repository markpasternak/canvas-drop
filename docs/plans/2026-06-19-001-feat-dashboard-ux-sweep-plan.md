...
**Revised build order from here:** U21 (description rename, foundational) → unified card + shared row (folds into U9/U10) → U11 (sparse strip) → U12/U13/U14/U15/U16 → U17 (gallery: unified card + list mode + featured/recent/sort) → U18 (admin lane) → U20 (onboarding/create brand) → docs refresh → **owner manual walkthrough** → U19 (marketing/landing images, LIGHT mode — **LAST**, only after the walkthrough) → final review → (await owner go-ahead before push).

> **R2b (owner directive):** landing/marketing image capture (U19) comes **dead last, after the owner's own manual walkthrough** — not before. The walkthrough may surface fixes; images are captured against the final UI.
