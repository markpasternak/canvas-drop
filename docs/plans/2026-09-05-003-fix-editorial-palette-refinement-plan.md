...
- [x] R1: Clean up light-mode paper tones, improve the separation of canvas/surface/controls, and retain a restrained deep teal accent. Make dark mode a related graphite palette with softer teal. Preserve readable text, semantic states, and focus indicators across all four skins.
- [x] R2: Keep shared tokens, dashboard CSS, public pages and generated brand artwork consistent. Review homepage, library, detail and docs in light/dark, plus narrow screens and alternative skins. Run palette parity/contrast checks, lint, typecheck, full dual-dialect/dashboard tests and build.
...
- [x] R5: Rewrite the GitHub README as the adoption entry point: explain the problem, product fit, first canvas, optional capabilities, and self-hosting choices. Keep the refreshed animated tour prominent and remove internal milestone, decision-record, roadmap, and status inventories from the reading flow. Verify instructions and links against the repository.
...
Tracking and release receipt: [issue #112](https://github.com/markpasternak/canvas-drop/issues/112). Local validation passed with 3,024 server/shared tests across both dialects and 738 dashboard tests; the eleven-frame animation and all documentation/marketing images were visually reviewed. Deployment and cleanup completion are recorded on the issue.
