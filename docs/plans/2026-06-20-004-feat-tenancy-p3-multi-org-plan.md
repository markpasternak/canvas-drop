...
- **KTD1 — DB-led orgs; config seeds the first.** Additional orgs are created via an operator
  route/CLI (name + domains), not env. The P1 boot-upsert becomes "ensure the seed org exists";
  beyond that, DB rows lead. Provisioning emits audit events.
- **KTD2 — Subdomain = the edge tenant selector; reconcile with the canvas URL mode.** The
  existing `CANVAS_DROP_URL_MODE` (path vs subdomain) governs **canvas** isolation; org-subdomain
  is a **second** axis. Decide the composition: most likely **`{org}.host` for the dashboard/app +
  `{slug}.{org}.host` (or `{org}.host/c/{slug}`) for canvases** — pick one and make
  `resolveRequest` org-aware. Personal/guests live on the bare `host`. This is the plan's hardest
  design question (brainstorm open-item #5) and gets its own design doc.
- **KTD3 — Membership becomes multi-org.** A user may belong to >1 org (multiple matching domains
  or invites). The principal carries a set (already true since P1); add an **active-org** notion for
  the dashboard workspace switcher and default scoping.
- **KTD4 — Per-org quotas.** Move AI/storage/rate limits from global/per-user to **per-org** where
  it matters; keep per-user caps as a floor. Config defines defaults; per-org overrides live in the
  org row (admin-set in Phase 4).
...
2. **U2 — Subdomain-per-org routing + origin isolation.** Org resolution at the edge; reconcile
   with the canvas URL mode; cookie/CSRF/CSP scoping per org subdomain; preserve the proxy-mode
   posture. (Own design doc — the URL wrinkle.)
3. **U3 — Per-org quotas/limits.** Per-org AI/storage/rate config + enforcement; per-user floor
   retained.
4. **U4 — Multi-org membership + active-org.** Membership as a set; active-org selection; default
   scoping; cross-org leak hardening.
