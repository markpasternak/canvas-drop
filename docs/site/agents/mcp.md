...
| `list_access` | The canvas's people list — the owner first, then named people, pending sign-in grants, and team grants — each with a `role` (`owner`/`editor`/`viewer`), a stable `id` (see Roles above) for `set_access_role` / `revoke_access`, `kind`, `email`, `name`, `userId`, `teamId`. |
