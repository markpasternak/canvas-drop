...
Two entry points, both POST to the same endpoint:

- `chat(messages, options)` — accumulate the full response.
- `stream(messages, options)` — iterate text as it arrives.

`options.model` is **required** and must be on the instance's model allowlist.
...
The default is `claude-haiku-4-5, claude-sonnet-4-6, claude-opus-4-8`. Asking
for a model that is not on the list throws with `code: "MODEL_NOT_ALLOWED"`
(HTTP 403). Use the exact model id as configured.
...
[Runtime API](/docs/api/runtime-api) for the underlying `POST /ai/chat`
