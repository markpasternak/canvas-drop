...
- **Audience** (not shown for **Use the API**). **Only me** is the default. A workspace canvas offers **Everyone in {workspace}** with a **List in Shared** toggle (off by default) so members can find it. A Personal canvas offers **Public link**, when the instance and your account allow public links, with an optional **Require password**. Changing the Workspace resets the audience to Only me. The audience is applied after the first publish; if that fails, the canvas stays private and the key dialog says so (**Save key and open Share**). The full ladder, named people, teams, and expiry live on the **Share** tab.
