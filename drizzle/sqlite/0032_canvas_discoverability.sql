...
CREATE TABLE `__new_canvases` (
	...
	`discoverability` text DEFAULT 'link_only' NOT NULL,
	...
	CONSTRAINT "canvases_discoverability_chk" CHECK("__new_canvases"."discoverability" in ('link_only', 'listed'))
);
--> statement-breakpoint
INSERT INTO `__new_canvases`("id", "slug", "slug_custom", "title", "description", "owner_id", "org_id", "access", "discoverability", "shared_expires_at", "gallery_listed", "gallery_templatable", "tags", "gallery_featured", "search_text", "gallery_published_at", "password_hash", "password_version", "spa_fallback", "preview_mode", "backend_enabled", "cap_kv", "cap_files", "cap_ai", "cap_realtime", "guest_ai_enabled", "guest_ai_cap", "api_key_hash", "status", "disabled_reason", "current_version_id", "cloned_from_canvas_id", "view_count", "last_viewed_at", "created_at", "updated_at", "deleted_at") SELECT "id", "slug", "slug_custom", "title", "description", "owner_id", "org_id", "access", CASE WHEN "access" = 'whole_org' AND "gallery_listed" = 1 THEN 'listed' ELSE 'link_only' END, "shared_expires_at", "gallery_listed", "gallery_templatable", "tags", "gallery_featured", "search_text", "gallery_published_at", "password_hash", "password_version", "spa_fallback", "preview_mode", "backend_enabled", "cap_kv", "cap_files", "cap_ai", "cap_realtime", "guest_ai_enabled", "guest_ai_cap", "api_key_hash", "status", "disabled_reason", "current_version_id", "cloned_from_canvas_id", "view_count", "last_viewed_at", "created_at", "updated_at", "deleted_at" FROM `canvases`;--> statement-breakpoint
