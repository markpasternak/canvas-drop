...
export const CONFIG_FIELDS: readonly ConfigField[] = [
  ...
  },
  {
    ...
    help: "Compatibility display name for custom email templates that still use {{instanceName}}. Seeded access emails avoid deployment and org branding.",
    type: "string",
    secret: false,
    editable: true,
  ...
  // an operator can confirm how delegated access emails are sent.
  {
    key: "email.driver",
    env: "CANVAS_DROP_EMAIL_DRIVER",
    group: "Email",
    label: "Email driver",
    help: "How sign-in and access emails are sent: log (dev), smtp, mailgun, or noop.",
    type: "enum",
  ...
  // ── Access notification emails (plan 003 phase 3; DB-only, no env) ────────
  {
    key: "email.invitesEnabled",
    env: "—",
    group: "Email",
    label: "Send access emails",
    help: "Master switch for sign-in and access notification emails. Off by default; only effective when a real email driver is configured above. A brand-new person can still sign in later, but they won't get an onboarding email.",
    type: "boolean",
    secret: false,
    editable: true,
  ...
  },
  {
    ...
    env: "—",
    group: "Email",
    label: "Notify on sign-in permit",
    help: "Email a person when an admin permits their email to sign in.",
    type: "boolean",
    secret: false,
    editable: true,
  ...
  },
  {
    ...
    help: "Email an existing user when they're added to a canvas's Specific-people list. No effect unless the master switch above is on and an email driver is configured.",
    type: "boolean",
    secret: false,
    editable: true,
  ...
  },
  {
    ...
    env: "—",
    group: "Email",
    label: "Notify on individual canvas access",
    help: "Email an existing user when they are deliberately given one-canvas access. No effect unless the master switch above is on and an email driver is configured.",
    type: "boolean",
    secret: false,
    editable: true,
  ...
  // ── Limits (quotas + rate limits) ────────────────────────────────────────
  {
    ...
    env: "—",
    group: "Limits",
    label: "Adds per user per hour",
    help: "Max add-person actions and onboarding/notification emails any one user may trigger per hour. Admins get a higher allowance.",
    type: "number",
    secret: false,
    editable: true,
  ...
  },
  {
    ...
    env: "—",
    group: "Limits",
    label: "Pending sign-ins per user",
    help: "Max pending sign-in grants any one user may have outstanding. This bounds standing access requests even under the hourly rate.",
    type: "number",
    secret: false,
    editable: true,
  ...
  },
  {
    ...
    env: "—",
    group: "Access",
    label: "Let members add brand-new emails",
    help: "Off by default. When OFF, members can add only people who can already sign in; permitting a brand-new external email is admin-only. When ON, any member may permit a new email to sign in while adding them.",
    type: "boolean",
    secret: false,
    editable: true,
