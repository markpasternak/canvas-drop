    ...
    <div className={compact ? "" : "[&_td]:py-4"}>
      <DataTable
        columns={[
          ...
          { header: "Access" },
          { header: "Status" },
          ...(!hiddenColumns.includes("size") ? [{ header: "Size", align: "right" as const }] : []),
          ...(!hiddenColumns.includes("usage")
            ? [{ header: "Usage", align: "right" as const }]
            : []),
          ...(!hiddenColumns.includes("activity") ? [{ header: "Last activity" }] : []),
          { srOnly: "Actions" },
        ]}
      >
        {canvases.map((c) => (
          <tr key={c.id} className="align-middle">
            ...
            <td className="px-3 py-2">
              <div className="max-w-72 space-y-1">
                ...
                <p className="text-xs text-muted">
                  {[
                    exposureFor(c).teamCount > 0 ? `${exposureFor(c).teamCount} teams` : null,
                    exposureFor(c).specificPeopleCount > 0
                      ? `${exposureFor(c).specificPeopleCount} people`
                      : null,
                    exposureFor(c).externalPeopleCount > 0
                      ? `${exposureFor(c).externalPeopleCount} external`
                      : null,
                  ]
                    .filter(Boolean)
                    .join(" · ") || "No additional people or teams"}
                </p>
                <details>
                  <summary className="cursor-pointer text-xs text-subtle">Access details</summary>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    ...
                    {exposureFor(c).pendingInviteCount > 0 && (
                      <Badge tone="warning">
                        {exposureFor(c).pendingInviteCount} pending access
                      </Badge>
                    )}
                  </div>
                </details>
            ...
            {!hiddenColumns.includes("activity") && (
