...
1. **Plan** — work comes from a plan in `docs/plans/`. **v1 is feature-complete: M1–M9 are all shipped and merged to `main`** — foundation (M1), hosting + deploy (M2), dashboard core (M3), canvas-management depth (M4), the editor + draft/publish version model on content-addressed storage (M5), the five primitives + browser SDK (M6: KV, files, `me()`, browser SDK), admin + hardening (M7), gallery (M8), and the AI proxy + realtime (M9). Every post-v1 feature is also merged: the canvas sharing access ladder (auth-delegated Add person grants + admin-gated public links), **Shared discovery** for canvases already opened to the viewer, usage stats, server-side list filters, the docs system (`/docs`, `/llms.txt`), clone-as-template, the primitives showcase, owner-chosen custom slugs, the **MCP server** (connect-once `/mcp`, OAuth 2.1, identity-scoped tools, on by default), the **staged content-addressed upload** (begin→stage→finalize), the signed-out **landing page**, the per-canvas **preview control** (auto-screenshot / off / owner-uploaded custom cover that survives publishes), **tenancy** (P1 org boundary, PR #56; P2 org teams + personal teams + auth-delegated invites, PR #58; access governance), the **authoring capability** (page-driven create, PR #70) + **managed shares** (PR #71), admin-flippable **design skins**, Anthropic **prompt caching**, per-version **ZIP download + delete** (PR #74), org-scoped **gallery listing** (PR #76), bulk archive/unarchive/delete, and the `popular` sort. The only open milestone is **M10 ops/packaging** (BUILD_BRIEF §16: Docker image + compose, backup/restore drill, single-VPS load test, IAP pilot). Of M10, the **Docker image + compose, vendor-neutral deploy docs, backup/restore tooling (`pnpm backup|restore|purge`, `apps/server/src/ops/`, `docs/ops.md`), the security review of the five invariants (`docs/security/`), README/quickstart, the 3 starter examples, and the secret-scan have shipped**; the backup/restore round-trip **drill** (an executed restore against real data), the single-VPS **load test**, and the **IAP colleague pilot** are still deferred. Also shipped: **MCP ↔ user parity** (`docs/plans/2026-06-17-001-feat-mcp-user-parity-plan.md`) — the MCP tool surface (46 tools) is now at **full parity with the dashboard owner/editor/viewer surface**: anything a user can do in the UI, an agent can do over MCP (settings/sharing via `update_canvas`, the preview cover via `update_canvas` previewMode + `set_canvas_preview`, the access/guest tools, `list_shared_canvases`, lifecycle, `clone_canvas`, `get_canvas_usage`, version export/deletion, and the whole draft-editor loop), each wrapping the same service layer with the same role checks (the [agent-native parity rule](#project-rules-load-bearing--from-build_briefmd)). Also shipped (2026-09-01): **canvas editor roles + ownership transfer** (`docs/plans/2026-09-01-1909-feat-canvas-editor-roles-plan.md`, issue #82) — viewer/editor roles on every canvas's people list (people and teams), one role resolver behind every owner gate, owner→editor transfer and admin reassign, per-file stale-save conflicts, owned-or-edited lists, and MCP parity driven by `tool-roles.ts`; learnings in `docs/solutions/2026-09-01-canvas-editor-roles-role-threading-and-transfer-atomicity.md`. Check `BUILD_BRIEF.md §16` and `README.md` Status for current status; don't free-style features that aren't in a plan.
...
- **Agent-native parity.** Anything a user can do in the dashboard UI, an agent must be able to do over the **MCP**. A new owner-facing capability is not "done" until its MCP tool ships alongside the UI. MCP tools **wrap the same service layer** the HTTP/management routes use — never a parallel implementation — and carry the same role gate (`loadManagementGrant` + `classifyMutability`, owner **or editor**, with each tool's minimum role declared once in `apps/server/src/mcp/tool-roles.ts`; a canvas the caller holds no role on reads as *not found*, an owner-only act by an editor reads `OWNER_ONLY`, §12.0) and the same audit events. The inventory and role-matrix tests fail on any tool registered without a table entry. (Admin-only cross-owner actions are the exception: they live on the dedicated admin routes, not the per-account MCP surface.)
...
pnpm test           # vitest via scripts/test-runner.mjs — BOTH dialects in-process (sqlite + pglite) + the dashboard suite
pnpm test:sqlite    # sqlite leg only (sets CANVAS_DROP_DB)
pnpm test:pg        # postgres leg only
pnpm test:dashboard # dashboard suite only
pnpm lint           # biome check
pnpm format         # biome check --write . (fixes + sorts imports); `pnpm lint` (biome check .) only reports
pnpm typecheck      # tsc --noEmit (root + sdk + dashboard)
pnpm build          # build all workspace packages (sdk, dashboard, server) via `pnpm -r build`
pnpm backup | restore | purge   # ops CLI (apps/server/src/ops/cli.ts); procedures in docs/ops.md
```

## Before pushing / merging

- **CI must be green on both dialects before merge.** The matrix (`.github/workflows/ci.yml`)
  runs lint + typecheck (and asserts the generated docs are current), test-sqlite,
  test-dashboard, test-postgres (+ real Postgres/MinIO), and build.
