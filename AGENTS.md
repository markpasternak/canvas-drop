...
- **CI must be green on both dialects before merge.** The matrix (`.github/workflows/ci.yml`)
  runs lint + typecheck (and asserts the generated docs and the bundled Mermaid renderer
  are current), test-sqlite, test-dashboard, test-postgres (+ real Postgres/MinIO), and
  build, plus an advisory (non-blocking) dependency audit.
