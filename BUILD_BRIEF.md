...
| D19 | Out of v1 | Data warehouse connector, CLI, agent skills, comments, automatic directory/search, custom backends, cron jobs, external users, public/anonymous sharing, multi-provider AI. |
...
| D23 | Share lifecycle | **Shares are revocable and optionally expiring.** Turning off "shared" or hitting an expiry instantly returns non-owners to 404; revocation also disconnects any open realtime sockets and invalidates password-gate cookies. No per-person allowlist in v1 (D4). |
...
3. **No unauthorized access.** A canvas is reachable only by its owner, or — if shared and not revoked/expired and any password is satisfied — by allowed org members. Owner-only canvases 404 to everyone else. Password-protected and unshared content stays closed.
...
**M6 — Primitives (F, G, I, J). [next]** KV (shared/user/increment), files, `me()`, browser SDK with mode auto-detection + docs + llms.txt. *(Realtime moves to M9 with AI.)* *Result: canvases become apps — pasted poll/form demos persist and read identity, zero-config.*
