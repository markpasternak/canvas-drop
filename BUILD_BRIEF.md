...
| D19 | Out of v1 | Data warehouse connector, CLI, agent skills, comments, automatic directory/search, custom backends, cron jobs, team/group visibility, standing external accounts / guest self-signup, multi-provider AI. *(Auth-delegated named people on a per-canvas allowlist and admin-gated static public links were added post-v1 — see D1/D4 and `docs/plans/2026-06-23-001-feat-auth-delegated-access-governance-plan.md`.)* |
...
4. Access ladder (D1/D4): per-canvas visibility rung — `private` (owner only) · `specific_people` (signed-in members plus auth-delegated pending emails) · `whole_org` (any authenticated member with the link) · `public_link` (admin-gated shareable link); each rung gates canvas-facing APIs too [v1]
