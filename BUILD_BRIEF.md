...
| D23 | Share lifecycle | **Shares are revocable and optionally expiring.** Lowering a canvas's rung, removing an allowlist or pending-grant entry, or hitting an expiry instantly returns the now-disallowed principal to 404 on the next request; revocation also disconnects any open realtime sockets and invalidates password-gate cookies. Legacy guest sessions retained from the old model are revoked on cutover or removal but no new guest credentials are minted. A per-person allowlist (signed-in members plus auth-delegated pending emails) is supported post-v1 (D4). |
...
**4.6 Owner-scoped, obscurity-supported.** Canvases belong to their creator. URLs are unguessable random slugs by default; owners may opt into a custom slug (post-v1), which trades obscurity for memorability — so the access rung, not the URL, is the real control. Sharing is explicit, revocable, and optionally time-boxed. Password protection adds a second lock where wanted.
...
21. Custom slugs / subdomain names — owner-chosen, DNS-safe, reserved-word-checked [post-v1] [done — supersedes the original "never": obscurity stays defense-in-depth only, default remains random, and a public+custom heads-up surfaces the trade-off]
...
11. Opt-in gallery listing for Public-link canvases and Whole-org canvases; Whole-org gallery opt-in also enables org-scoped discoverability [v1]
...
12. Opt-in gallery: Public-link canvases and explicitly listed Whole-org canvases can appear with owner-provided title/description/tags; Whole-org listing is visible only within the canvas's home org [v1]
...
3. **No unauthorized access.** A canvas is reachable only by: its **owner or an editor** (an org member holding the editor role on its people list, directly or through an editor-role team, resolved live — editors are owner-equivalent on the management/editor surface except the owner-only acts: delete, transfer, and the guest-AI opt-in; ownership moves only by the owner's transfer to an editor or an admin's reassign between other members); at `whole_org`, any allowed org member; at `team`, a signed-in member of a granted team; at `specific_people`, a signed-in principal on its allowlist (an existing user or a pending email after that exact email has verified through the configured auth); at `public_link`, anyone — but static-only and only while the instance public-link switch is on and the owner account has not been revoked. Pending grants have no auth power by themselves. All subject to not-revoked/not-expired and any password (only the owner is never prompted). **An admin gets no special access to canvases it doesn't own**: for another user's canvas an admin is treated as an ordinary org member — the rung applies (a non-owned `private`, non-granted `team`, or non-granted `specific_people` canvas 404s for an admin too) and a password-protected rung prompts the admin like anyone else. Cross-owner admin authority is limited to the dedicated admin routes (the all-canvases list + disable/enable/restore + reassign-owner, which moves ownership between *other* members and never confers content access on the acting admin); it never extends to canvas content, the owner-or-editor management/editor surface (view/edit/deploy/settings/sharing/delete), the runtime API, or realtime. *Recorded exception:* the deploy/authoring API's update-in-place and revoke routes admit an admin as an owner (a decision locked in the managed-shares plan) — kept as is and named here rather than silently restored either way. Everything else 404s; an owner-only act attempted by an editor is an explicit `OWNER_ONLY` refusal (the editor legitimately knows the canvas exists), while a principal with no role keeps the opaque 404; `private` and unshared content stays closed; an external person can never reach a canvas they were not granted. An anonymous public visitor has no identity and gets no primitives.
...
**M8 — Gallery. [done]** Opt-in gallery browse/listing for Public-link canvases and organization-scoped Whole-org canvases with owner metadata. Enabling gallery listing on Whole org also opts it into member discovery; it never lists automatically from the access rung alone. Needs apps worth surfacing, so it follows primitives + admin.
