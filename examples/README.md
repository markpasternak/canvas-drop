...
Example canvases that show what a canvas *is* and what the SDK does. Each is
...
Enable Backend and Key-value storage for `participant-input`. Open as an owner or
editor to set a question, then as a viewer to submit. A viewer cannot edit the
question, inspect another response, or publish totals. Shared counter mutations
in `kv-counter` and `showcase` also require owner/editor. For the showcase AI demo,
choose an AI audience in Backend; viewer AI is off by default.
