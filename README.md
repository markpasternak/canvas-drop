...
- **AI agents are first-class authors.** Canvas code runs zero-config, the deploy API ships from day one, and the SDK contract lives on one agent-readable page (`/llms.txt`). An agent can write a canvas and ship it with no human in the loop — over the keyed deploy API, the installable [agent skill](docs/site/agents/skill.md), or a connect-once **[MCP server](docs/site/agents/mcp.md)** that signs in through the org's own login (OAuth) and exposes identity-scoped tools. The MCP surface has full dashboard parity (32 tools): create/list/get/update, deploy and staged deploy, read-back and verify, versions, rollback, unpublish, archive/unarchive, delete, set capabilities, set slug, regenerate the deploy key, clone, usage, the sharing/guest tools (grant/revoke/list access, resend invite), and the full draft editor loop (read/write/rename/delete draft files, publish, restore).
