...
- **AI agents are first-class authors.** Canvas code runs zero-config, the deploy API ships from day one, and the SDK contract lives on one agent-readable page (`/llms.txt`). An agent can write a canvas and ship it with no human in the loop — over the keyed deploy API, the installable [agent skill](docs/site/agents/skill.md), or a connect-once **[MCP server](docs/site/agents/mcp.md)** that signs in through the org's own login (OAuth) and exposes identity-scoped tools. The MCP tool set today is a deliberate subset of what an owner can do in the dashboard (create, list, get, deploy/staged-deploy, read-back/verify, versions, rollback, unpublish, archive/unarchive, delete, set capabilities, set slug, regenerate the deploy key); broader dashboard↔MCP parity is in progress.
...
- ✅ **Agent MCP server** — connect-once remote MCP at `/mcp` (OAuth 2.1 via the instance's own login, PKCE + dynamic client registration), a deliberate subset of identity-scoped owner tools (create/list/get, deploy + staged deploy, read-back/verify, versions, rollback, unpublish, archive/unarchive, delete, set capabilities, set slug, regenerate key — 18 tools in all), on by default and disablable with `CANVAS_DROP_MCP=off`
...
M10 is the only open milestone — everything above it is merged to `main` with CI green on both dialects. The one actively in-flight plan is MCP↔dashboard tool parity: the manage tools (set capabilities, set slug, regenerate key, delete, archive/unarchive) have landed; the sharing/guest, clone, usage, `update_canvas`, and draft-editor tools are not yet built. See [`docs/plans/`](docs/plans/) and [`BUILD_BRIEF.md` §16](BUILD_BRIEF.md).
