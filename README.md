...
- **A deliberate sharing ladder.** Private, Specific people (signed-in members or pending external people), a Team (a self-serve group — personal *friends & family* or org-attached), Whole org, or a static Public link that admins can disable globally or revoke per account. Revocable and optionally time-boxed. Invites are **auth-delegated**: a brand-new person gets access on their first sign-in through your configured auth — no app-owned passwords or magic-link accounts.
